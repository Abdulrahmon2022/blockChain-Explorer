import { Block, Transaction, AddressInfo, TokenInfo, TokenTransfer, Validator } from "@/types";
...
for (let i = 0; i < 50; i++) {
  ...
  const parentHash = i === 0 ? generateMockHash("0xbb", blockNumber - 1) : mockBlocks[i - 1].hash;
  ...
  for (let t = 0; t < txCount; t++) {
    ...
    let to: string | undefined = mockAddresses[(txIndex + 2) % mockAddresses.length];
    if (txIndex % 7 === 0) {
      // Contract creation
      to = undefined;
    }
    ...
    const priorityFee = 1500000000n + BigInt((txIndex * 100000000) % 3000000000); // ~1.5-4.5 Gwei
    ...
    const tx: Transaction = {
      ...
      input: to === undefined 
        ...
        : (txIndex % 3 === 0 ? "0xa9059cbb000000000000000000000000" + mockAddresses[(txIndex + 1) % mockAddresses.length].substring(2).padStart(64, '0') : "0x"),
      timestamp,
    ...
    if (txIndex % 3 === 0 && to !== undefined) {
      ...
      mockTokenTransfers.push({
        ...
        from,
        to,
        value: token.type === 'ERC-20' ? transferValue * (10n ** BigInt(token.decimals === 0 ? 1 : token.decimals - 2)) : undefined,
  ...
  mockBlocks.push({
    ...
    extraData: "0x476574682f76312e31302e31352d737461626c652f6c696e75782f676f312e31372e35",
...
export const mockValidators: Validator[] = [
  { address: mockAddresses[8], name: "Lido: Validator 1", totalProposed: 12054, slashed: false, active: true },
  { address: mockAddresses[9], name: "Coinbase: Validator 2", totalProposed: 9845, slashed: false, active: true },
  { address: "0x002c918f1aef20092ab7168d1847bc18cc489e21", name: "Binance Pool 3", totalProposed: 4321, slashed: false, active: true },
  { address: "0x981628172cde3281907cb3ccb83cc987bbcb3cc7", name: "Kiln 1", totalProposed: 2314, slashed: false, active: true },
  { address: "0xbcda0c293cfeb38cc8fbc2e11894d8961726a117", name: "Solo Staker: 0xbcda", totalProposed: 450, slashed: false, active: true },
];
